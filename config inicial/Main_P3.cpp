...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
	   ...
	   view = glm::translate(view, glm::vec3(0.0f,0.0f,-20.0f)); //en z se echa hacia atrás
		model = glm::rotate( model, 0.5f, glm::vec3(2.0f, 3.0f, 4.0f ) ); // use to compare orthographic and perspective projection
...
//--------------------
		...
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-7.0f, -7.0f,-12.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(4.0f, 4.0f, 4.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.5f, 1.5f, 1.5f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::rotate(model, 0.5f, glm::vec3(7.0f, 3.0f, 2.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.5f, 1.5f, 1.5f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		glBindVertexArray(0);//esto va al final
